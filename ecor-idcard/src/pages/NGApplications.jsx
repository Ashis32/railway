...
import pendingWN from '../data/pendingWN';

export default function WNApplications() {
  const [dateFrom, setDateFrom] = useState('');
  const [dateTo, setDateTo] = useState('');
  const [status, setStatus] = useState('');

  // (You can later filter based on these states)
  const filteredData = pendingWN;

  return (
    <div>
      {/* Page Title */}
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-blue-800 mb-1">📂 Pending Applications (Non-Gazetted Staff)</h2>
        <p className="text-sm text-gray-600">Below are all pending identity card requests (Work Notice based).</p>
      </div>
